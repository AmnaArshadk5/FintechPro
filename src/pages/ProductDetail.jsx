...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const transformed = rawProductsData.map(mapToFinancialProduct);
    setAllProducts(transformed);

    const foundItem = rawProductsData.find((item) => item.id === parseInt(id));
    if (foundItem) {
      const mappedData = mapToFinancialProduct(foundItem);
      setProduct(mappedData);
      setAmount(mappedData.minInvestment);
    } else {
  ...
  if (!product) return (
    <div style={{ padding: "150px 20px", textAlign: "center", minHeight: "80vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div>
        <div style={{ fontSize: "48px", marginBottom: "16px" }}>⏳</div>
        <p style={{ color: "#64748b", fontSize: "16px" }}>Loading Asset Data...</p>
      </div>
    </div>
  );

  const yearlyProfit = (amount * product.expectedReturn) / 100;
  const isInPortfolio = portfolio?.items?.some(item => item.id === product.id);

  const getDecisionInsight = (p) => {
    const insights = [];
    
    if (p.riskLevel === 'high') insights.push("🔥 Aggressive Growth: Best for investors comfortable with significant volatility.");
    else if (p.riskLevel === 'low') insights.push("🟢 Capital Safety: Ideal for preserving initial investment with steady returns.");
    else insights.push("⚖️ Balanced Strategy: Moderate exposure for steady growth with acceptable risk.");

    if (p.liquidity === 'locked') insights.push("🔒 Capital Lock-in: Funds are not easily withdrawable—early exit may incur penalties.");
    if (p.timeHorizon === 'long') insights.push("📅 Long-Term Hold: Recommended for 5+ years to maximise compound growth.");
  ...
  const getProfileMatch = () => {
    ...
    const matchCount = [riskMatch, horizonMatch, liquidityMatch, budgetMatch].filter(m => m).length;
    const matchPercentage = (matchCount / 4) * 100;

    return { matchCount, matchPercentage, riskMatch, horizonMatch, liquidityMatch, budgetMatch };
  };
  ...
  const handleAdd = () => {
    if (amount < product.minInvestment) {
      alert(`⚠️ Minimum investment for this product is PKR ${product.minInvestment.toLocaleString()}`);
      return;
    }
  ...
  return (
    <div style={{ padding: "120px 20px", maxWidth: "1000px", margin: "auto", fontFamily: "sans-serif" }}>
      <Link to="/products" style={{ color: "#64748b", textDecoration: "none", fontWeight: "600", display: "inline-flex", alignItems: "center", marginBottom: "24px" }}>
        ← Back to Marketplace
      </Link>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "40px", marginTop: "20px" }}>
        ...
        <div>
          ...
          {profileMatch && (
            <div style={{
              ...
              <p style={{ margin: "0 0 8px 0", fontSize: "12px", fontWeight: "600", color: profileMatch.matchPercentage >= 75 ? "#166534" : profileMatch.matchPercentage >= 50 ? "#92400e" : "#991b1b" }}>
                {profileMatch.matchPercentage >= 75 ? "✅ Great Match" : profileMatch.matchPercentage >= 50 ? "⚠️ Partial Match" : "❌ Poor Match"} with Your Profile ({profileMatch.matchPercentage.toFixed(0)}%)
              </p>
              <div style={{ fontSize: "11px", color: "#64748b", lineHeight: "1.5" }}>
                Risk: {profileMatch.riskMatch ? "✓" : "✗"} | Horizon: {profileMatch.horizonMatch ? "✓" : "✗"} | Liquidity: {profileMatch.liquidityMatch ? "✓" : "✗"} | Budget: {profileMatch.budgetMatch ? "✓" : "✗"}
              </div>
            </div>
          )}
          ...
          <button 
            onClick={handleAdd}
            disabled={isAdded || amount < product.minInvestment}
            style={{ 
              width: "100%", 
              padding: "16px", 
              background: isAdded ? "#16a34a" : amount < product.minInvestment ? "#cbd5e1" : "#0f172a", 
              color: "white", 
              border: "none", 
              borderRadius: "8px", 
              fontWeight: "700", 
              cursor: isAdded || amount < product.minInvestment ? "not-allowed" : "pointer", 
              fontSize: '16px', 
              transition: '0.3s'
            }}
            onMouseOver={(e) => !isAdded && amount >= product.minInvestment && (e.target.style.background = "#1e293b")}
            onMouseOut={(e) => !isAdded && amount >= product.minInvestment && (e.target.style.background = "#0f172a")}
          >
      ...
      {amount >= product.minInvestment && (
        <div style={{ marginTop: "50px", padding: "30px", background: "linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%)", borderRadius: "16px", border: '1px solid #bbf7d0', textAlign: 'center' }}>
          ...
          <p style={{ color: '#166534', margin: '0 0 20px 0', fontSize: "14px" }}>
            Based on PKR <strong>{amount.toLocaleString()}</strong> investment with <strong>{product.expectedReturn}%</strong> annual return:
          </p>
          ...
          <small style={{ color: '#166534', fontWeight: '600', fontSize: "13px" }}>
            Total after 1 year: <strong>PKR {(amount + yearlyProfit).toLocaleString()}</strong>
          </small>
          <div style={{ marginTop: "16px", fontSize: "12px", color: "#166534", lineHeight: "1.5" }}>
            ⚠️ <strong>Disclaimer:</strong> This is a projection based on current returns. Actual returns may vary. Past performance doesn't guarantee future results.
          </div>
        </div>
      )}
    </div>
